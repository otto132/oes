...
async function main() {
  ...
  await prisma.accessLog.deleteMany().catch(() => {});
  ...
  await prisma.lead.createMany({ data: [
    { id: 'l1', company: 'BASF SE', domain: 'basf.com', source: 'Signal', signalId: 's1', type: 'Industrial', country: 'Germany', region: 'DACH', stage: 'New', pain: 'Likely manual GoO tracking for PPA volumes.', moduleFit: ['GoO Procurement', 'Reporting'], scoreFit: 72, scoreIntent: 65, scoreUrgency: 75, scoreAccess: 30, scoreCommercial: 80, confidence: 0.65, ownerId: 'u1', certMgmtType: 'Excel', certRegistries: ['UBA'], itIntegrations: ['SAP ERP'], certPainPoints: 'Industrial buyer with PPA — likely tracking GoO volumes manually in Excel alongside energy procurement.' },
    { id: 'l2', company: 'Uniper SE', domain: 'uniper.com', source: 'Signal', signalId: 's3', type: 'Utility', country: 'Germany', region: 'DACH', stage: 'Researching', pain: 'Expanding GoO trading desk. Likely outgrowing manual tools.', moduleFit: ['GoO Trading', 'GoO Issuance'], scoreFit: 80, scoreIntent: 60, scoreUrgency: 70, scoreAccess: 35, scoreCommercial: 75, confidence: 0.65, ownerId: 'u1', certMgmtType: 'ETRM', etrmSystem: 'OpenLink Endur', certRegistries: ['UBA', 'AIB Hub'], itIntegrations: ['OpenLink Endur', 'Bloomberg'], certPainPoints: 'Hiring GoO Trading Analyst suggests scaling operations beyond current Endur capabilities for cert tracking.' },
    { id: 'l3', company: 'TotalEnergies Trading', domain: 'totalenergies.com', source: 'Signal', signalId: 's4', type: 'Trader', country: 'France', region: 'Western Europe', stage: 'Qualified', pain: 'New GoO desk for Nordics — greenfield. Needs full stack.', moduleFit: ['GoO Trading', 'GoO Issuance', 'Reporting', 'API Integration'], scoreFit: 90, scoreIntent: 85, scoreUrgency: 88, scoreAccess: 45, scoreCommercial: 85, confidence: 0.85, ownerId: 'u2', certMgmtType: '', certRegistries: [], itIntegrations: [], certPainPoints: 'Greenfield GoO desk for Nordics — no incumbent tooling. Need full cert management stack from day one.' },
    { id: 'l4', company: 'Enel Green Power', domain: 'enelgreenpower.com', source: 'Conference', type: 'Utility', country: 'Italy', region: 'Southern Europe', stage: 'New', pain: '60 GW renewable capacity globally. Scale problem.', moduleFit: ['GoO Issuance', 'Reporting'], scoreFit: 65, scoreIntent: 35, scoreUrgency: 55, scoreAccess: 25, scoreCommercial: 90, confidence: 0.35, ownerId: 'u3', certMgmtType: 'ETRM', etrmSystem: 'Molecule', certRegistries: ['GSE', 'AIB Hub'], itIntegrations: ['Molecule ETRM', 'SAP'], certPainPoints: '60 GW renewable capacity across multiple markets. Scale of cert issuance likely overwhelming current Molecule setup.' },
  ]});
  console.log('  ✓ 4 leads');

  // ── Opportunities ──────────────────────────────
  await prisma.opportunity.createMany({ data: [
    { id: 'o1', name: 'Vattenfall GoO Platform', stage: 'Proposal', amount: 380000, probability: 55, closeDate: daysFromNow(108), healthEngagement: 40, healthStakeholders: 75, healthCompetitive: 60, healthTimeline: 50, nextAction: 'Follow up on revised GoO volume proposal', nextActionDate: daysFromNow(1), accountId: 'a1', ownerId: 'u1', source: 'Referral' },
    ...
    { id: 'o4', name: 'Fortum I-REC Pilot', stage: 'Discovery', amount: 145000, probability: 15, closeDate: daysFromNow(200), healthEngagement: 55, healthStakeholders: 40, healthCompetitive: 80, healthTimeline: 70, nextAction: 'Send platform overview ahead of call', nextActionDate: daysAgo(2), accountId: 'a2', ownerId: 'u2', source: 'Direct' },
    { id: 'o5', name: 'E.ON UK REGO Module', stage: 'Proposal', amount: 185000, probability: 55, closeDate: daysFromNow(47), healthEngagement: 80, healthStakeholders: 70, healthCompetitive: 65, healthTimeline: 75, nextAction: 'Proposal review call with Sarah Hughes', nextActionDate: daysFromNow(11), accountId: 'a3', ownerId: 'u2', source: 'Expansion' },
