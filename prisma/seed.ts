...
async function main() {
  ...
  await prisma.opportunity.createMany({ data: [
    { id: 'o1', name: 'Vattenfall GoO Platform', stage: 'Proposal', amount: 380000, probability: 65, closeDate: new Date('2026-06-30'), healthEngagement: 40, healthStakeholders: 75, healthCompetitive: 60, healthTimeline: 50, nextAction: 'Follow up on revised GoO volume proposal', nextActionDate: new Date('2026-03-15'), accountId: 'a1', ownerId: 'u1' },
    { id: 'o2', name: 'E.ON Germany GoO Procurement', stage: 'SolutionFit', amount: 460000, probability: 50, closeDate: new Date('2026-05-31'), healthEngagement: 65, healthStakeholders: 55, healthCompetitive: 40, healthTimeline: 60, nextAction: 'Follow up with Kai on API feasibility', nextActionDate: new Date('2026-03-14'), accountId: 'a3', ownerId: 'u1' },
    { id: 'o3', name: 'Statkraft GoO Trading Pilot', stage: 'Qualified', amount: 290000, probability: 35, closeDate: new Date('2026-07-31'), healthEngagement: 30, healthStakeholders: 35, healthCompetitive: 70, healthTimeline: 80, nextAction: 'Prepare pilot scope via Erik Sandvik referral', nextActionDate: new Date('2026-03-18'), accountId: 'a4', ownerId: 'u1' },
    { id: 'o4', name: 'Fortum I-REC Pilot', stage: 'Discovery', amount: 145000, probability: 20, closeDate: new Date('2026-09-30'), healthEngagement: 55, healthStakeholders: 40, healthCompetitive: 80, healthTimeline: 70, nextAction: 'Send platform overview ahead of call', nextActionDate: new Date('2026-03-12'), accountId: 'a2', ownerId: 'u2' },
    { id: 'o5', name: 'E.ON UK REGO Module', stage: 'Proposal', amount: 185000, probability: 65, closeDate: new Date('2026-04-30'), healthEngagement: 80, healthStakeholders: 70, healthCompetitive: 65, healthTimeline: 75, nextAction: 'Proposal review call with Sarah Hughes', nextActionDate: new Date('2026-03-25'), accountId: 'a3', ownerId: 'u2' },
    { id: 'o6', name: 'Axpo ELcert Expansion', stage: 'Negotiation', amount: 95000, probability: 80, closeDate: new Date('2026-03-31'), healthEngagement: 90, healthStakeholders: 85, healthCompetitive: 95, healthTimeline: 60, nextAction: 'Send revised pricing — 8.5% discount', nextActionDate: new Date('2026-03-20'), accountId: 'a5', ownerId: 'u1' },
    { id: 'o7', name: 'RWE GoO Issuance', stage: 'Contacted', amount: 320000, probability: 10, closeDate: new Date('2026-12-31'), healthEngagement: 20, healthStakeholders: 25, healthCompetitive: 80, healthTimeline: 90, nextAction: 'Discovery call March 20', nextActionDate: new Date('2026-03-20'), accountId: 'a6', ownerId: 'u2' },
  ]});
  ...
  const taskData = [
    { id: 't1', title: 'Follow up Vattenfall — GoO proposal response', status: 'Open' as const, priority: 'High' as const, due: new Date('2026-03-10'), source: 'Pipeline Hygiene Agent', accountId: 'a1', ownerId: 'u1', goalId: 'g1', assigneeIds: ['u1'] },
    { id: 't2', title: 'Prepare Statkraft pilot scope document', status: 'InProgress' as const, priority: 'High' as const, due: new Date('2026-03-18'), source: 'Meeting Notes', accountId: 'a4', ownerId: 'u1', goalId: 'g2', reviewerId: 'u2', assigneeIds: ['u1', 'u3'] },
    { id: 't3', title: 'Send Fortum platform overview', status: 'Open' as const, priority: 'High' as const, due: new Date('2026-03-12'), source: 'Manual', accountId: 'a2', ownerId: 'u2', assigneeIds: ['u2'] },
    { id: 't4', title: 'Follow up Kai Mueller — API feasibility', status: 'Open' as const, priority: 'High' as const, due: new Date('2026-03-08'), source: 'AI Suggested', accountId: 'a3', ownerId: 'u1', assigneeIds: ['u1'] },
    { id: 't5', title: 'Send Axpo revised pricing — ELcert', status: 'Open' as const, priority: 'Medium' as const, due: new Date('2026-03-20'), source: 'Manual', accountId: 'a5', ownerId: 'u1', goalId: 'g3', assigneeIds: ['u1'] },
    { id: 't6', title: 'Review 3 new signals', status: 'Open' as const, priority: 'Medium' as const, due: new Date('2026-03-11'), source: 'Signal Hunter Agent', ownerId: 'u1', assigneeIds: ['u1', 'u2'] },
  ];
  ...
  await prisma.taskComment.createMany({ data: [
    { text: 'Lars was responsive last week — try calling directly', taskId: 't1', authorId: 'u2', createdAt: new Date('2026-03-09T14:00:00Z') },
    { text: '@Juuso — Kai mentioned SSO concern in last call. Address in follow-up.', mentions: ['u1'], taskId: 't4', authorId: 'u3', createdAt: new Date('2026-03-08T09:00:00Z') },
  ]});
  console.log('  ✓ 2 task comments');

  // ── Activities ─────────────────────────────────
  await prisma.activity.createMany({ data: [
    { type: 'Email', summary: 'ELcert module pricing — revised offer sent', detail: "Sent 8.5% multi-module discount proposal. Awaiting Erik's sign-off.", source: 'Outlook Sync', accountId: 'a5', authorId: 'u1', createdAt: new Date('2026-03-10T09:14:00Z') },
    { type: 'Meeting', summary: 'Discovery call — Fortum I-REC registry needs', detail: 'Anna confirmed interest. Key concern: CFO approval needed.', source: 'Calendar Sync', accountId: 'a2', authorId: 'u2', createdAt: new Date('2026-03-09T14:00:00Z') },
    { type: 'Email', summary: 'API integration feasibility — follow-up sent', detail: 'Sent SAP connector documentation. Awaiting IT team review.', source: 'Outlook Sync', accountId: 'a3', authorId: 'u1', createdAt: new Date('2026-03-08T16:30:00Z') },
    { type: 'Meeting', summary: 'Intro call via Erik Sandvik referral', detail: 'Good intro. Torben aware of Eco-Insight via Axpo. Pilot concept resonated.', source: 'Calendar Sync', accountId: 'a4', authorId: 'u1', createdAt: new Date('2026-03-07T10:00:00Z') },
    { type: 'Email', summary: 'Commercial proposal sent — E.ON Germany', detail: 'Full proposal submitted. €460K ARR, 3-year term.', source: 'Outlook Sync', accountId: 'a3', authorId: 'u1', createdAt: new Date('2026-02-28T11:00:00Z') },
    { type: 'Note', summary: 'Revised GoO volume proposal sent', detail: 'Updated commercial terms with volume thresholds. No response yet.', source: 'Manual', accountId: 'a1', authorId: 'u1', createdAt: new Date('2026-02-19T09:00:00Z') },
  ]});
  console.log('  ✓ 6 activities');

  // ── Queue Items ────────────────────────────────
  await prisma.queueItem.createMany({ data: [
    { id: 'q1', type: 'outreach_draft', title: 'Cold outreach to BASF SE — GoO Procurement', accName: 'BASF SE', agent: 'Outreach Drafter', confidence: 0.82, confidenceBreakdown: { relevance: 0.92, personalization: 0.74, timing: 0.81 }, reasoning: 'BASF appeared in PPA signal with Ørsted. Industrial buyer profile matches ICP.', sources: [{ name: 'Reuters Energy', url: 'https://reuters.com/energy/orsted-basf-ppa' }, { name: 'LinkedIn', url: 'https://linkedin.com/in/basf-energy' }], payload: { to: 'Michael Braun <m.braun@basf.com>', subject: 'Eco-Insight — GoO certificate management for BASF', body: 'Hi Michael,\n\nI noticed BASF recently signed a 186 MW offshore wind PPA with Ørsted...' }, priority: 'Normal', createdAt: new Date('2026-03-11T09:30:00Z') },
    { id: 'q2', type: 'lead_qualification', title: 'Qualify Uniper SE as GoO Trading lead', accName: 'Uniper SE', agent: 'Lead Qualifier', confidence: 0.71, confidenceBreakdown: { icp_fit: 0.80, intent: 0.65, entity_match: 0.68 }, reasoning: 'Uniper hiring GoO Trading Analyst. Signal confidence is medium.', sources: [{ name: 'LinkedIn Jobs', url: 'https://linkedin.com/jobs/uniper-goo' }], payload: { company: 'Uniper SE', type: 'Utility', country: 'Germany', stage: 'Researching', scores: { f: 80, i: 60, u: 70, a: 35, c: 75 }, pain: 'Expanding GoO trading desk' }, priority: 'Normal', createdAt: new Date('2026-03-10T14:00:00Z') },
    { id: 'q3', type: 'enrichment', title: 'Update Vattenfall brief — procurement timeline intel', accName: 'Vattenfall Nordic AB', accId: 'a1', agent: 'Account Enricher', confidence: 0.76, confidenceBreakdown: { source_quality: 0.85, relevance: 0.72, freshness: 0.70 }, reasoning: 'LinkedIn post from Lars Eriksson mentions "evaluating 3 vendors for GoO platform".', sources: [{ name: 'LinkedIn Post', url: 'https://linkedin.com/posts/lars-eriksson' }], payload: { field: 'whyNow', before: 'Budget cycle Q2. Champion Lars actively evaluating.', after: 'Budget cycle Q2. Lars confirmed evaluating 3 vendors (LinkedIn post Mar 8). Procurement process active.' }, priority: 'High', createdAt: new Date('2026-03-10T11:00:00Z') },
    { id: 'q4', type: 'task_creation', title: 'Schedule follow-up with Torben (Statkraft) via Erik', accName: 'Statkraft Markets', accId: 'a4', agent: 'Pipeline Hygiene', confidence: 0.68, confidenceBreakdown: { engagement_decay: 0.90, relationship_path: 0.55, priority: 0.60 }, reasoning: 'No response from Torben in 4 days since intro call.', sources: [{ name: 'Activity Log', url: null }, { name: 'Contact Graph', url: null }], payload: { task: 'Ask Erik Sandvik to nudge Torben Rasmussen re: pilot discussion', due: '2026-03-14', pri: 'High' }, priority: 'Normal', createdAt: new Date('2026-03-11T07:00:00Z') },
    { id: 'q5', type: 'outreach_draft', title: 'Sequence Step 2 — Enel Green Power follow-up', accName: 'Enel Green Power', agent: 'Outreach Drafter', confidence: 0.55, confidenceBreakdown: { relevance: 0.70, personalization: 0.42, timing: 0.52 }, reasoning: 'Low confidence: no response to initial outreach. Contact role unverified.', sources: [{ name: 'Sequence: Cold Outreach — GoO Pain', url: null }], payload: { to: 'Energy Certs Team <certs@enelgreenpower.com>', subject: 'Following up — GoO issuance at scale', body: 'Hi there,\n\nI reached out last week about Eco-Insight\'s GoO issuance platform...' }, priority: 'Low', createdAt: new Date('2026-03-11T08:00:00Z') },
  ]});
  console.log('  ✓ 5 queue items');

  // ── Inbox Emails ───────────────────────────────
  await prisma.inboxEmail.createMany({ data: [
    { subject: 'Re: ELcert Module Pricing', fromEmail: 'erik.sandvik@axpo.com', fromName: 'Erik Sandvik', preview: "Thanks for the revised offer. 8.5% works for us. Let's finalize in the QBR.", receivedAt: new Date('2026-03-10T16:20:00Z'), isUnread: true, classification: 'positive_reply', classificationConf: 0.95, isLinked: true, accountId: 'a5', accountName: 'Axpo Nordic AS' },
    { subject: 'RE: SAP Integration — E.ON', fromEmail: 'kai.mueller@eon.com', fromName: 'Kai Mueller', preview: 'IT team has follow-up questions on REST endpoints. Can you send API docs?', receivedAt: new Date('2026-03-09T11:40:00Z'), isUnread: true, classification: 'question', classificationConf: 0.91, isLinked: true, accountId: 'a3', accountName: 'E.ON Energy Markets' },
    { subject: 'Eco-Insight Interest', fromEmail: 'maria.hoffmann@rwe.com', fromName: 'Maria Hoffmann', preview: "We'd like to learn about GoO issuance capabilities for our European portfolio.", receivedAt: new Date('2026-03-08T09:15:00Z'), isUnread: false, classification: 'positive_reply', classificationConf: 0.88, isLinked: false, domain: 'rwe.com', accountName: 'RWE Renewables Europe', accountId: 'a6' },
    { subject: 'Out of Office: Johan Lindgren', fromEmail: 'johan.lindgren@vattenfall.com', fromName: 'Johan Lindgren', preview: 'I am out of office until March 17. For urgent matters contact Lars Eriksson.', receivedAt: new Date('2026-03-09T07:00:00Z'), isUnread: false, classification: 'auto_reply', classificationConf: 0.99, isLinked: true, accountId: 'a1', accountName: 'Vattenfall Nordic AB' },
    { subject: 'Speaker Invitation — Nordic Energy Summit', fromEmail: 'events@nordicenergy.org', fromName: 'Nordic Energy Events', preview: "We'd like to invite Eco-Insight to speak at our May summit.", receivedAt: new Date('2026-03-07T14:30:00Z'), isUnread: false, classification: 'meeting_request', classificationConf: 0.82, isLinked: false },
    { subject: 'Partnership Inquiry — Green Certificate Platform', fromEmail: 'contact@certiverde.io', fromName: 'CertiVerde', preview: 'We are a new GoO marketplace and would love to discuss API integration.', receivedAt: new Date('2026-03-06T10:00:00Z'), isUnread: false, classification: 'new_domain', classificationConf: 0.75, isLinked: false, domain: 'certiverde.io' },
  ]});
